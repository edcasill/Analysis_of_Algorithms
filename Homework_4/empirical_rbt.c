...
/*
================================================================
TU IMPLEMENTACIÓN DE RBT (COPIADA EXACTAMENTE)
================================================================
*/
...
void RB_insert_fixup(RBT *T, Node *z)
{
    while (z->parent->color == RED)
    {
        if (z->parent == z->parent->parent->left) //if z's parent a left child?
        {
            ...
            {
                /*case 1*/
                // Error corregido: El 'parent' debe ser BLACK, no RED
                z->parent->color = BLACK; // MODIFICADO
                y->color = BLACK;
                z->parent->parent->color =RED;
                z = z->parent->parent; // Mover z para el siguiente ciclo
            }
            else
            {
                if (z == z->parent->right)
...
void RB_insert(RBT *T, int key)
{
    ...
    while (x != T->nil) // descend until reaching the sentinel
    {
        ...
        else if (z->key > x->key) // Evitar duplicados (o manejarlos)
        {
            x = x->right;
        }
        else // z->key == x->key
        {
            // Clave duplicada, no insertar y liberar memoria
            free(z);
            return;
        }
    }
    ...
    //z->left = T->nil; // Hecho en createNode
    //z->right = T->nil; // Hecho en createNode
    //z->color = RED; // Hecho en createNode
    RB_insert_fixup(T,z);
}
